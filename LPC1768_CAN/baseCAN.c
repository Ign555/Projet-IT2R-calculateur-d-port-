...
void CANthreadT(void const *argument)
{
	...
	data_buf [0] = 0xAA; // data � envoyer � placer dans un tableau de char
	
	while (1) {

		// Code pour envoyer trame Id 0x0f6
		commande = Lecture_GPIO();
		data_buf[0] = commande;
...
void CANthreadR(void const *argument)
{
	...
	{		
		...
		sprintf(texte,"ID: %d Rx: %d", 0x0b6, (short)data_buf);
		GLCD_DrawString(100,150,texte);
		//.............
